<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Connexion</title>
    <link rel="stylesheet" href="styles.css">
</head>

<body>
    <div class="container" id="app">
        <div class="image-section">
            <img src="https://images.pexels.com/photos/8636603/pexels-photo-8636603.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" alt="Femme avec un foulard">
        </div>
        <div class="login-section">
            <div class="login-header">
                <h1>CONNEXION</h1>
            </div> 
            <form role="form" method="POST" action="{{ route('login') }}"> 
                @csrf
                <div class="form-group">
                    <label for="email">Nom d'utilisateur</label>
                    <input type="text" id="email" name="email" required>
                </div>
                <div class="form-group">
                    <label for="password">Mot de passe</label>
                    <input type="password" id="password" name="password" required>
                </div>
                <div class="forgot-password">
                    <a href="#">Mot de passe oublié ?</a>
                </div>
                <button type="submit" class="submit-btn">SOUMETTRE</button>
            </form>
            <div class="create-account">
                <a href="#">Créer un compte</a>
            </div>
            <div class="alternative-login">
                <p>Ou</p>
                <div class="social-login">
                    <div class="social-btn">
                        <img src="data:image/png;base64," alt="Connexion Google">
                    </div>
                    <div class="social-btn">
                        <img src="https://img.freepik.com/vecteurs-premium/illustration-artistique_929495-41.jpg?semt=ais_hybrid" alt="Connexion Facebook">
                    </div>
                </div>
            </div>
        </div>
    </div>
    <script type="module">
        import { createApp } from 'https://cdn.jsdelivr.net/npm/vue@3.2.41/dist/vue.esm-browser.prod.js';

        createApp({
            data() {
                return {
                    username: '',
                    password: '',
                    errors: []
                };
            },
            methods: {
                handleSubmit(event) {
                    event.preventDefault();
                    this.errors = [];

                    if (!this.username || !this.password) {
                        this.errors.push('Veuillez remplir tous les champs.');
                    }

                    if (this.errors.length === 0) {
                        console.log('Connexion réussie avec:', { username: this.username, password: this.password });
                        // Ajoutez ici votre logique de connexion
                    }
                }
            }
        }).mount('#app');
    </script>
</body>
</html>
